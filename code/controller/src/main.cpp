...
static constexpr size_t PACKET_SIZE = 17; // 2 sync + 1 seq + 4 slider + 6 air + 3 btn + 1 checksum
...
static void scan_slider()
{
  ...
  for (int i = 0; i < 4; i++) slider_bits[i] = 0;
...
void scan_air()
{
  ...
  bool temp_is_air = (digitalRead(0) == LOW);
  digitalWrite(1, LOW);
  air_is_on = false;

  digitalWrite(LED_BUILTIN, temp_is_air ? HIGH : LOW);

  static uint8_t temp_current_air = 0;

  if (temp_is_air)
  {
    if (temp_current_air < 5) temp_current_air++;
  }
  else
  {
    if (temp_current_air > 0) temp_current_air--;
  }

  if (temp_current_air == 0 && !temp_is_air)
  {
    for (int i = 0; i < 6; i++) air_beams[i] = 0;
  }
  else
  {
    for (int i = 0; i < 6; i++)
    {
      air_beams[i] = (i <= temp_current_air) ? 1 : 0;
    }
  }
}
